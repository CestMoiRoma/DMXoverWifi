...
def main():
    ...
    # The wiki is not carried. It was, and it cost 13 KB of a partition at 84%
    # to ship a copy of something the UI links to on GitHub anyway, where it is
    # also searchable, current and not a build behind.
    ...
    # Guarded, so the headless builds do not carry 40 KB of a page they never
    # serve.
    ...
    print("pack_web: page %d bytes from %d (css %d, js %d, %.0f%% smaller), %.1f KB of flash%s"
          % (len(packed), len(raw), len(css), len(js),
             100.0 * (1 - len(packed) / float(len(raw))), len(packed) / 1024.0,
